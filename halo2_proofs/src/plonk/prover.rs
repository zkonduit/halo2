    ...
    T: Debug,
{
    ...
    log::debug!("instances.len() = {}", instances.len());
    log::debug!("instances[0].len() = {}", instances[0].len());
    log::debug!(
    ...
    log::debug!("hash_into(transcript) done");
    ...
    log::debug!("shuffles = {:?}", &shuffles);

    // Commit to the vanishing argument's random polynomial for blinding h(x_3)
    let vanishing = vanishing::Argument::commit(params, domain, &mut rng, transcript)?;

    log::debug!("vanishing = {:?}", &vanishing);
    ...
    log::debug!("transcript after advice evals = {:?}", &transcript);
    ...
    log::debug!("transcript after fixed evals = {:?}", &transcript);
    ...
    log::debug!("instances = {:?}", &instances.clone().collect::<Vec<_>>());
